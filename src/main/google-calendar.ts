...
import { getOrCreateDatabaseClient, type DatabaseClient } from '@/db/client';
...
interface CreateGoogleCalendarServiceOptions {
    authServerRuntime?: ServerRuntime;
...
export function createGoogleCalendarService({
    ...
    let authServerPromise: Promise<Awaited<ReturnType<ServerRuntime['ensureStarted']>>> | undefined;
    ...
    const getConnectionRuntime = async () => {
        connectionRuntimePromise ??= (async () => {
            const repository = new (await import('@/db/google-repository')).GoogleRepository(
                await client,
            );
